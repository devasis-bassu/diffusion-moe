...
Note the naming collision to watch for: `CosineMoELayer`'s "cosine similarity" (its *routing metric*, operating on raw residual-stream vectors) is unrelated to `DiffusionMoELayer`/`PilotMoEBlock`'s own `cosine` flag (§4, stage 1) for the diffusion map's *kernel distance metric*. Both use the word "cosine" for genuinely different things at different stages of the pipeline — a `router="diffusion"` layer with `cosine=True` and a `router="cosine"` layer are unrelated settings that happen to share a name.
