...
### 6.6 Compute-matching a routed layer against dense: the active-FFN-FLOPs formula

Any dense-vs-MoE comparison in this codebase needs both sides running the same active FLOPs/token to mean anything — parameter count alone is misleading, since most of a routed layer's parameters are inert for any given token. `ExpertFFN`'s hidden width is `ffn_dim // n_experts × overlap_factor` (not `ffn_dim` itself — that's the *total* capacity split across `n_experts`), and each token activates `top_k` routed experts plus the always-on `shared_expert` (§6.1) when enabled. So:

```
active_ffn_width_per_token = (top_k + int(use_shared_expert)) × (ffn_dim // n_experts × overlap_factor)
```

A compute-matched dense comparison sets the plain `TransformerBlock`'s `ffn_dim` (`model.ffn_dim`) to this value directly, since dense's `FeedForward` uses its configured `ffn_dim` as-is with no division. At this project's `base_config.yaml` defaults (`n_experts=8`, `top_k=2`, `ffn_dim=4096`, `overlap_factor=1.0`, `use_shared_expert=true`): `active_ffn_width = 3 × 512 = 1536`, dramatically less than the `ffn_dim=4096` a naive dense-baseline config would use. `phase2_training_report.md` §8.9 found the uncorrected 2.7x compute gap had been silently favoring dense in every prior comparison in that report (§6.4, §6.5, §8.6, §8.8) — worth checking explicitly, not assumed, in any future dense-vs-routed comparison.

### 6.7 Expert-separation / duplication check: token attribution + weight cosine similarity

Neither load-balance loss in this codebase (`coefficient_of_variation_loss`, `switch_load_balance_loss`) penalizes anything beyond marginal per-expert dispatch *frequency* — a router that consistently pairs experts into redundant duplicate groups would score as perfectly "balanced" under either, since both are satisfied by symmetric duplication just as well as by genuine functional diversity. Two complementary checks, run against a trained checkpoint (`phase2_training_report.md` §8.8 has the worked example, Switch layer 0, `n_experts=8`):

- **Token-identity overlap**: `scripts/expert_attribution.py --checkpoint <path> --layers <idx>` (needs the checkpoint's actual data config passed as overrides — see §6.4) produces each expert's top-N most-routed tokens. Pairwise Jaccard overlap of these top-token sets across all expert pairs is a cheap proxy for functional overlap — genuinely redundant experts would show overlap approaching 1.0 for at least one pair; §8.8's result averaged 0.08 (range 0.00-0.21).
- **Weight-space similarity**: flatten each expert's `down_proj.weight` (or any single projection, held consistent across experts) and compute pairwise cosine similarity; compare against a random-initialization baseline of the same shape as the null hypothesis (high-dimensional random vectors are already near-orthogonal, so "near zero" alone doesn't prove non-redundancy — the informative comparison is trained-vs-random, not trained-vs-zero). §8.8's trained similarities (mean -0.0005, max 0.007) were statistically indistinguishable from the random baseline (mean 0.0002, max 0.003).

Neither check is definitive on its own (token-overlap says nothing about internal weight structure; weight-cosine in a ~500K-dim space is a weak test for redundancy expressed via internal hidden-unit permutation rather than raw parameter alignment) — a functional-output-correlation check (comparing experts' actual outputs on a shared batch of real activations, which is invariant to the internal-permutation blind spot) would be the more rigorous version if either check's result is ambiguous or a future run's finding needs to be trusted more strongly.
