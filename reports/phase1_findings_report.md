...
| `src/diffusion_moe/routing/router.py` | Added noisy top-k gating (`noise_std`, off by default) to `DiffusionRouter` — see §3.5 |
| `src/diffusion_moe/data/dataset.py` | Added `local_data_files` to `StreamingTextDataset` — bypasses Hub streaming (a recurring flaky-CDN stall, hit on two different datasets across this investigation) by reading already-downloaded local parquet files directly — see §3.5 and `methodology.md` §4 |
