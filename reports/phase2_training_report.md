...
**Recommendation, revised again**: not ready for a scaled-up run, and the seed-repeat result (§6.5) removes the most likely objection to that conclusion — this was not one unlucky comparison. Before spending more compute on the current diffusion-MoE approach at all: (a) profile and reduce the CPU-bound Nyström/eigensolve cost specifically — a 15-21x wall-clock penalty needs a very large quality win to ever justify itself, and right now the result runs the other direction; (b) revisit whether all-layers replacement was the right configuration to keep pursuing — Phase 1's original oracle-ceiling work recommended a *selective* few layers, not all 24, which this investigation deliberately set aside to answer "what happens if every layer routes." That question has now been answered, fairly conclusively, at this scale: worse and less consistent than not routing at all. The selective-layer approach remains untested by this phase and is the more promising next direction, not a bigger version of what was just run.
