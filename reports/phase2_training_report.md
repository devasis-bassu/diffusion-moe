...
**§8 (added after the above was first written) goes further and identifies why.** A single, data-driven selective layer (§8.1) closes most of the all-layers gap on its own — but investigating *why* the routed layer still lagged dense surfaced a real bug (`centroid_refresh_steps` was silently measured in micro-batches, refitting twice as often as configured, §8.2) and, more importantly, a structural finding: `task_loss` never recovers to its pre-refit level within a full refit-to-refit window (§8.3) — the diffusion map is fighting a representation that's continuously reshaped by the same gradient descent the routing depends on, not just suffering a brief coordinate-sign hiccup. A targeted fix for the latter (§8.4) helped only at the margin. The decisive test: `SwitchMoELayer` — already in this codebase, a standard continuously-trained linear router with no periodic refit at all — lands within a few percent of dense at the identical layer selection (§8.6), and two follow-up runs (§8.8) confirm this generalizes: Switch gets *closer* to dense when scaled to all layers (the opposite of diffusion, which got 4.6x worse), and is seed-robust like dense (sub-1% seed variance) rather than seed-sensitive like diffusion (44-52%). **§8.9 then found that comparison itself wasn't fair**: dense's baseline config ran 2.7x more active FFN compute per token than Switch the entire time. Compute-matched, dense and Switch are statistically *tied* (`val_ppl` 100.33 vs 100.4) — Switch isn't beating diffusion by being a better architecture at equal resources, it's tied with dense while using 1/2.7 the compute and diffusion is losing to both. **This phase's honest bottom line, updated**: the infrastructure now works; the diffusion-geometric router's underperformance is specifically the periodic-hard-refit design (not sparse routing or this project's scale, per §8.6/§8.8); and separately, sparse MoE (Switch) hasn't yet demonstrated a quality advantage over a compute-matched dense model at this scale/budget — only compute parity, at 3x the parameter cost (§8.9). Per-project decision at the close of this phase: further diffusion-router development is on hold pending a real structural fix to the moving-target problem (§8.3); whether Switch-style MoE earns its complexity is a separate, still-open question pending a larger scale point.
...
**Revised once more, after §8.9**: the dense baseline this whole recommendation leaned on was never compute-matched to Switch — it ran 2.7x more active FFN FLOPs/token. Compute-matched, dense and Switch tie exactly (§8.9), which doesn't change the diffusion-router verdict above (diffusion still loses to both, and by a wide margin) but does soften the case for Switch specifically: at this scale and token budget, sparse MoE routing hasn't yet shown a quality edge over dense at equal active compute, only parity at 3x the total parameter cost. Before recommending Switch-style MoE for the eventual scaled-up model (as opposed to just using it as the diffusion router's replacement in any further diffusion-specific work), the open question is whether that gap opens up in MoE's favor at a larger scale point or token budget — consistent with the standard finding in the literature that MoE's advantage tends to emerge with more data/parameters, not at this project's current 20M-token, ~sub-billion-parameter regime. Untested here.
...
Together with §8.6, this closes out the comparison cleanly across every axis tested — single-layer quality, all-layers scaling, and seed stability — and all three favor Switch over the diffusion router by a wide, consistent margin.

**A real gap worth flagging, then checked empirically.** Neither load-balance loss in this codebase (`coefficient_of_variation_loss`, `switch_load_balance_loss`) penalizes anything beyond marginal per-expert dispatch frequency — a router that consistently pairs experts into redundant duplicate groups would score as perfectly "balanced" under either loss, since both are satisfied by symmetric duplication just as well as by genuine diversity. No orthogonality/diversity term exists anywhere in this codebase, across any of the four router variants. Checked directly against the one surviving Switch checkpoint (`winter-moon-31`, layer 0, `n_experts=8`, `top_k=2` — the `checkpoints/` directory is overwritten per-run, so `faithful-jazz-29`'s all-layers checkpoint is no longer available) using `scripts/expert_attribution.py` plus a direct weight comparison:

- **Token specialization**: each expert's top-15 routed tokens (pooled over 20 held-out batches) are visibly distinct — e.g. expert 1 is dominated by `.` and capitalized proper-noun starts (`P`/`W`/`D`/`G`), expert 4 by function words (`the`/`of`/`in`/`by`), expert 3 by numerals and `was`. Mean pairwise Jaccard overlap across all 28 expert pairs' top-token sets: 0.08 (range 0.00-0.21) — far from the ~1.0 two truly redundant experts would show.
- **Weight-space similarity**: pairwise cosine similarity between experts' `down_proj` weight matrices is statistically indistinguishable from a random-initialization baseline (trained: mean -0.0005, max 0.007; random-init baseline of the same shape: mean 0.0002, max 0.003) — no anomalous alignment between any pair.
- **Load**: fairly even in training (`load_loss` ~0 for all three §8.6/§8.8 Switch runs), though the held-out attribution sample shows real spread (1,045-3,974 tokens across the 8 experts, 3.8x) — not collapsed, but not perfectly uniform either.

For this run, the duplication concern doesn't appear to be materializing — the 8 experts look genuinely differentiated by both routing outcome and learned weights, despite no explicit mechanism preventing it. This isn't proof it can never happen (only one checkpoint checked, and weight-cosine in a ~500K-dim space is a weak test for redundancy expressed via internal hidden-unit permutation rather than raw parameter alignment — a functional-output-correlation check on shared activations would be the more rigorous version if this needs revisiting), but it's no longer a purely theoretical worry left unchecked.
...
### 8.9 The dense-vs-Switch comparison was never compute-matched — corrected

Every dense-vs-MoE comparison so far (§6.4, §6.5, §8.6, §8.8) used `model.ffn_dim=4096` for dense (the config default) against Switch's actual active per-token FFN width, without checking whether those were comparable in the first place. They weren't: `ExpertFFN`'s hidden width is `ffn_dim // n_experts × overlap_factor = 4096 // 8 × 1.0 = 512`, and each token activates `top_k` routed experts plus the always-on shared expert (§3), so Switch's true active FFN compute per token is `(top_k + 1) × 512 = 1,536` — dense at `ffn_dim=4096` was running **2.7x more active FLOPs/token** than Switch the entire time. The 93.2-vs-100.4 `val_ppl` gap in §8.8 partly (perhaps entirely) reflected that compute disparity, not an architectural advantage for dense.

Corrected: a dense run with `model.ffn_dim=1536`, exactly matching Switch's active FFN width, same data/budget/seed as every other run in §8:

| Config | Active FFN FLOPs/token | Total FFN params/layer | `task_loss` | `val_ppl` |
|---|---|---|---|---|
| Dense, `ffn_dim=4096` (uncorrected baseline) | 12.6M | 12.6M | 4.80 | 93.2 |
| **Dense, `ffn_dim=1536`** (`wild-eon-32`, compute-matched) | **4.7M** | **4.7M** | **4.877** | **100.33** |
| Switch, all layers (`faithful-jazz-29`) | 4.7M | 14.2M (3x dense) | 4.871 | 100.4 |

At matched active compute, dense and Switch are statistically indistinguishable (`val_ppl` 100.33 vs 100.4, `task_loss` 4.877 vs 4.871 — well within this project's measured seed-to-seed noise floor, §6.5/§8.8). Switch carries 3x the total FFN parameters of compute-matched dense for that tie, not a win — the extra routed capacity isn't translating into a quality advantage at this scale/token budget. This is a materially different conclusion from §8.6/§8.8's framing: Switch isn't "nearly as good as dense," it's *exactly* as good as a dense model given the same compute, and the sparse-MoE structure isn't yet earning its added complexity and 3x parameter cost. Whether that changes at a larger token budget or model scale (where MoE's literature advantage typically shows up) is untested — this is one data point, not a scaling curve.

### 8.10 Literature context: is this tie a novel finding, or the expected one?

`SwitchMoELayer` is a faithful (if simplified) implementation of Fedus, Zoph & Shazeer 2021 ("Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity") — the same learned-linear-gate-plus-load-balance-loss design underlies most production sparse-MoE LLMs since (GShard, GLaM, Mixtral, DeepSeek-MoE). §8.9's tie is consistent with, not contradicted by, the literature on this architecture:

- The field's routed-vs-dense quality advantage is itself scale-dependent (Clark et al. 2022, "Unified Scaling Laws for Routed Language Models" — already the source cited in §7 for why a single small-scale comparison shouldn't be over-trusted either way). MoE's advantage over compute-matched dense is a claim about the large-scale asymptote, not guaranteed to hold at every budget.
- MoE models are typically *more* data-hungry per active parameter than dense at a fixed compute budget (Krajewski et al. 2024, "Scaling Laws for Fine-Grained Mixture of Experts"), meaning an under-trained small-budget comparison like this project's 20M tokens is exactly the regime where MoE would be expected to underperform its asymptotic potential, not a neutral test of it.
- This implementation is also missing two components the literature found necessary for MoE to reach its potential at scale: an **expert capacity factor** (the original Switch Transformer caps tokens routed to each expert per batch and drops overflow, which both bounds compute and forces real load balancing under a hard constraint) and a **router z-loss** (added in the ST-MoE follow-up, Zoph et al. 2022, specifically to stabilize router logits; found to measurably improve quality). Neither exists anywhere in this codebase (confirmed by grep across `src/`) — plausible, untested candidates for narrowing the tie into a real Switch advantage even at this project's current scale, before reaching for a bigger scale point.

Net read: §8.9's tie is the expected outcome given the literature, not a surprising negative result about this specific implementation — and it points at concrete, cheap next experiments (capacity factor, z-loss) before concluding anything about whether MoE "works" here at all.
...
## 10. Phase Closed: Final Status

This phase is closed as of this report. Summary of where each track landed, and what would justify reopening it:

**Diffusion-geometric router — on hold, needs a structural fix, not further tuning.** Decisively behind both dense and Switch on every axis tested this phase: quality (§6.4, §8.1, §8.6), cost (§8.6 — ~70% slower than Switch even at one layer), and seed-robustness (§6.5, 44-52% variance vs. Switch/dense's sub-1%). Root-caused, not just observed: the periodic hard refit is fighting a representation continuously reshaped by the same gradient descent the routing depends on (§8.3), and a targeted fix for the coordinate-discontinuity symptom (§8.4, sign-alignment) helped only at the margin — confirming the problem is the moving-target dynamic itself, not refit mechanics. **What would reopen this track**: a real structural redesign (e.g. a soft/continuous update to the geometric embedding instead of a periodic hard refit, or detaching fewer signals from the graph so the embedding target moves less) — not another round of hyperparameter or refit-cadence tuning, both of which this phase already tried.

**Switch-style sparse MoE — ties compute-matched dense here; advantage over dense not yet demonstrated at this scale.** Nearly matches dense at a naive (uncorrected) comparison (§8.6, §8.8) and ties it exactly once compute is actually matched (§8.9) — at 3x the parameter cost for that tie. Consistent with the literature's expectation that MoE's edge is scale-dependent and this project's 20M-token budget is smaller than where that edge typically shows up (§8.10), and this implementation is missing two components (expert capacity factor, router z-loss) the literature found relevant at scale. **What would reopen this track**: either a cheap in-place experiment (capacity factor and/or z-loss at the current scale, per §8.10) or a genuine second scale point (larger token budget/model size) to see whether the tie breaks in Switch's favor as literature would predict — this phase did not run either.

**What this phase leaves behind, concretely**: a working all-layers training pipeline (seven real bugs fixed, §9), a data-driven layer-selection methodology (§8.1/§6.7) and a compute-matching methodology (§8.9/methodology.md §6.6) reusable for any future comparison on this codebase, and a duplication/separation-checking methodology (§8.8/methodology.md §6.7) for auditing any router's learned experts. Nothing here is a dead end — both open tracks have a specific, named next step, not just "try harder."

---

## Appendix: Open Questions Carried Forward

- Does the depth-progression / sequence-start routing pattern (§6.6) persist, sharpen into content-sensitivity, or dissolve entirely at a later training step and a larger token budget? **Partially answered for layer 0 by §6.7**: by the final checkpoint it had moved past pure position into a real content-correlated split (digit vs. word tokens on one seed), though the specific feature wasn't identical on the other seed. Layer 6 specifically (the depth-progression layer) wasn't re-checked at the final checkpoint — still open. (Diffusion-specific; low priority given §10's disposition on that track.)
- Should `nu_sep` (currently unchanged at 0.05) be measured the same way `mu_load` was (§6.2)? `sep_loss` showed real instability of its own (swings from -3.5 to -10.6 within ~120 steps in the collapsed run) that was not directly investigated — it may be a downstream symptom of the same collapse dynamic, or a separate miscalibration. (Diffusion-specific; low priority given §10's disposition on that track.)
- The self-tuning (per-point adaptive) bandwidth design discussed but not implemented (the "Option 2" alternative to §2.2's global live-refresh) remains a candidate if staleness-driven instability recurs even with the current fix. (Diffusion-specific; low priority given §10's disposition on that track.)
- `NystromDiffusionMap`'s own k-means/eigensolve randomness still has a hardcoded `random_state=42`, independent of `cfg.seed` (§9) — the seed-repeat check in §6.5 varied model init and data-adjacent RNG state, but not this. Worth wiring through too if the seed-sensitivity finding in §6.5 needs isolating further (is it driven by init, by the diffusion-map's own landmark selection, or both). (Diffusion-specific; low priority given §10's disposition on that track.)
- **Switch expert-capacity factor and router z-loss (§8.10)** — neither implemented; both plausible candidates for closing the compute-matched tie (§8.9) in Switch's favor without needing a larger scale point. Highest-priority open item if this track is reopened.
- **A second scale point for Switch vs. dense (§8.10, §7)** — the compute-matched tie (§8.9) is one data point at one (small) budget; the literature's MoE-advantage claim is about the large-scale asymptote specifically. Untested here.
- **Functional-output-correlation check for expert duplication (§8.8, methodology.md §6.7)** — the token-overlap and weight-cosine checks both have known blind spots (internal hidden-unit permutation invariance in particular); a shared-activation output-correlation check would be the more rigorous version if the duplication question needs revisiting with higher confidence than one checkpoint currently supports.
