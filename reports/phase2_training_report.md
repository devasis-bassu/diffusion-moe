...
The honest answer is: five real, previously-undiscovered bugs, one badly-miscalibrated hyperparameter, and — after fixing all of them — one clean, complete training run that demonstrates the architecture can learn, route without permanently collapsing, and finish without crashing. That run is real progress, not a finished result — and a same-budget dense-model control run, added at the end of this phase, decisively outperformed it (§6.4). See §7 for the full picture and what would change that conclusion.
...
### 6.5 What token-level routing actually looks like (early-training snapshot)
...
**What this phase established**: the infrastructure-level failure modes are found and fixed — NaN divergence, checkpoint resume, disk exhaustion, and (the big one) the initialization bug that made every prior run's absolute numbers meaningless. One full run now completes cleanly end-to-end with sane losses throughout. And, now that the missing control has been run (§6.4): **at this scale and budget, the dense baseline beats the all-layers diffusion-MoE model decisively — better `task_loss` (4.80 vs. 7.08), an order of magnitude better `val_ppl` (93.2 vs. 974.6), and ~15x less wall-clock time for the same token budget.**

**Remaining open questions**, now that the dense-baseline question (previously listed first here) is answered:

1. **`n=1` on both sides.** Only one run of each configuration has completed. This project's own history (Phase 1 §3.5: seed-42 vs. seed-123 producing comparably severe collapse onto *different* experts) is a direct precedent for why a single run of either isn't yet strong evidence of a reproducible gap, though a 10x perplexity gap is large enough that ordinary seed variance is an unlikely full explanation.
2. **The final near-zero `load_loss` coincides with the learning rate being fully decayed.** It is plausible routing balance was still actively, robustly resolving itself — and equally plausible it simply froze wherever it happened to be as gradients vanished toward the end of the cosine schedule, which would not be evidence of a stable equilibrium at all.
3. **20M tokens is a smoke-test budget**, not a real pretraining scale, and the standard case for why sparse MoE is worth its complexity (Clark et al. 2022's routed-vs-dense scaling laws, discussed earlier in this investigation) is specifically about quality-per-active-FLOP *at scale* — this comparison is nowhere near the regime that literature has actually demonstrated an advantage in. §6.4's result does not show diffusion-MoE is a bad idea in general; it shows this specific comparison offers no evidence for it.

**Recommendation, revised**: not ready for a scaled-up run — and now for a sharper reason than before. Before spending more compute on the current diffusion-MoE approach at all, the ~15x wall-clock gap (almost entirely the CPU-bound Nyström/eigensolve pipeline, not GPU compute) and the perplexity gap both need either to close or to have a specific, articulated reason to expect them to close only at larger scale. Candidate next steps, in order of what would most change the conclusion: (a) a repeat of both runs with a different seed, to rule out this being unusually good/bad luck on either side; (b) profiling and reducing the CPU-bound geometry cost specifically, since a 15x wall-clock penalty would need a very large quality win to ever justify itself; (c) if (a) and (b) don't change the picture, treating this as a real negative result for the all-layers configuration specifically, and revisiting whether a selective (not uniform) layer-replacement strategy — the original recommendation from `phase1_findings_report.md`'s oracle-ceiling work, before this investigation's "let all layers use it and see" detour — is the better path forward.
...
- Does the depth-progression / sequence-start routing pattern (§6.5) persist, sharpen into content-sensitivity, or dissolve entirely at a later training step and a larger token budget?
